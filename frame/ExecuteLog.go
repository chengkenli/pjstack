...
func ExecuteLog() {
	...
	if util.P.IP == "" {
		...
		if strings.Contains(strings.Join(tools.FrontendsIP(db), ","), util.P.IP) {
			util.P.Te = "fe"
		}
		if strings.Contains(strings.Join(tools.BackendsIP(db), ","), util.P.IP) {
			util.P.Te = "be"
		}
	...
	for i, ip := range host {
		...
		if util.P.Rtar {
			...
			if len(util.P.Filename) != 0 {
				f = nil
				f = append(f, strings.ReplaceAll(fmt.Sprintf("%s/%s", path, util.P.Filename), " ", ""))
			}
		...
		if len(util.P.Filename) != 0 {
			file := strings.ReplaceAll(fmt.Sprintf("%s/%s", path, util.P.Filename), " ", "")
